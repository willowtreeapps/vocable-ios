...
public class Pulse: NSObject {
    ...
    public class Configuration: CustomDebugStringConvertible {
        /// Minimum value of error to agree that system already reached value it aims for (assuming there is no overshoot)
        private(set) var minimumValueStep: CGFloat
        /// Proportional gain
        private(set) var Kp: CGFloat
        /// Integral gain
        private(set) var Ki: CGFloat
        /// Derivative gain
        private(set) var Kd: CGFloat
        ...
        public func apply(_ configuration: Configuration) {
            self.minimumValueStep = configuration.minimumValueStep
        ...
        public var debugDescription: String {
            return "Configuration: minValueStep \(minimumValueStep), kP: \(Kp), kI: \(Ki), kD: \(Kd)"
        }
    }
    ...
    var setPointChangedClosure: ((CGFloat) -> Void)?
    ...
    @objc func tick() {
        ...
        while dt > Constants.MaxTimeDelayDuration {
            ...
            dt -= Constants.MaxTimeDelayDuration
        }
