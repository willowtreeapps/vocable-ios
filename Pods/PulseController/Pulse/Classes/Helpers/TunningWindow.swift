...
class TunningWindow: UIWindow {
    ...
    private static var _shared: TunningWindow? = nil
...
class TuningContainerViewController: UIViewController {
    ...
    func addTuningViewController(for pulse: Pulse, minValue: CGFloat, maxValue: CGFloat) {
        ...
        let tunningViewController: TunningViewController = TunningViewController(pulse: pulse, configuration: tunningViewConfiguration, closeClosure: { [weak self] (sender) in
            pulse.outputClosure = proxyOutputClosure
            pulse.setPointChangedClosure = nil
            self?.removeTuningViewController(for: pulse)
        }, configurationChanged: { (sender, newConfiguration) in
            pulse.configuration = newConfiguration
        })

        // Listen to changes in `output` and new `setPoint`
        pulse.outputClosure = { newValue in
            tunningViewController.pulseOutputValue = newValue
            proxyOutputClosure(newValue)
        }

        pulse.setPointChangedClosure = { newValue in
